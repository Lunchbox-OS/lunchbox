...
upgrades belongs in `/etc/sway/shepherd.conf.d/*.conf` instead. For example, to
...
| What | Where | Owner |
| --- | --- | --- |
| usage history, quotas, audit log, policy | `/var/lib/lunchboxd/state/<user>/` | `shepherd-state` |
| BLE admin record, unbond queue, reset sentinel — the *device's*, not a user's | `/var/lib/lunchboxd/admin/` | `shepherd-state` |
...
shepherd-state:shepherd-state /var/lib/lunchboxd/state` after restoring fixes it.
...
sudo -u shepherd-state sqlite3 /var/lib/lunchboxd/state/kiosk/lunchboxd.db
```
...
Nothing shepherd installs is a dpkg *conffile*: there is no file here whose
local edits an upgrade will preserve or ask you about. That is deliberate.

- `/etc/sway/shepherd.conf` is **generated**, with the rewrites described under
  ...
  `/etc/sway/shepherd.conf.d/*.conf` — left unmanaged on purpose — instead.
- `/etc/systemd/system/lunchbox-stated@.{service,socket}` are shepherd's units,
...
where shepherd's own copy sits.
