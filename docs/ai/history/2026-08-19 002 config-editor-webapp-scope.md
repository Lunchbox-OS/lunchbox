# Config editor — scope and implementation

> Status: **built, not yet deployed.** Phases 0–2 landed on
> `feat/config-editor` (six commits, unpushed at time of writing); phases 3–5
> are not started. The hosting pipeline is written but has never run — the
> Cloudflare project and its two secrets do not exist yet.
>
> The plan below is preserved as it was written and approved. Differences
> between it and what shipped are called out in "As built" at the end. No issue
> filed.
...
- **Which host.** `origin` is Forgejo (`git.armeafamily.com`), not GitHub, so
  ...
  the repo. *Open question below — settled as Cloudflare; see "Follow-up:
  hosting".*
...
Phase 5 is then: add `src-tauri/`, point `frontendDist` at `dist-standalone/`,
...
Two of these resolved differently in practice. The embedded-bundle row is
superseded — the editor is not routed from the management UI at all, so nothing
of it reaches `dist/` and the ~4% binary cost is currently zero. And the
boundary guard shipped as a small script rather than a bundle-size budget. Both
are covered below.

**Open questions** (all three answered during implementation — see "As built"):
...
Beyond the tab, which has its own section below:

| Planned | Shipped |
|---|---|
| The two-target `rsbuild.config.ts` in phase 0 | **Phase 1.** There is nothing for a standalone target to build until `standalone.tsx` exists, so phase 0 stops at `shepherd build config-wasm`. |
| Phase 1 "deployed to the static host" | **After phase 2, as its own commit**, and tag-triggered rather than continuous — see "Follow-up: hosting". |
| Open question 3, snap granularity | **15 minutes.** `ScheduleGrid` and `ScheduleEditor` both take a `step` prop defaulting to 15, so 5 or 30 is a one-line change at the call site rather than a rewrite. |

### Commits

Six, split so each phase stands alone (newest first):

```
ci(config-editor): publish the standalone editor to Cloudflare Pages
feat(scripts): add `shepherd dev webui` to run the dev server
feat(config-editor): phase 2 — the rest of the schema
feat(config-editor): phase 1 — the graphical editor
feat(config-editor): phase 0 — the comment-preserving document model
docs(config-editor): scope a graphical, comment-preserving config editor
```

Each was verified against a tree isolated with `git stash push -u --keep-index`,
so the gates ran on exactly the staged commit rather than on the finished
working tree — phase 0's vitest run really does show only the six duration
tests.

Three files span phases 1 and 2 (`EntryDetail.tsx`, `EntriesPage.tsx`,
`ConfigApp.tsx`). Rather than commit a phase 1 that does not typecheck, each was
staged in a genuine reduced form: no Advanced tab or token gate, no
drag-and-drop, no Device tab. `package.json` and its lockfile are the exception
and land whole in phase 0, since splitting a lockfile means inventing
`npm install` states that never existed.
...
The pipeline expects a Direct Upload project named `shepherd-config-editor`
whose **production branch is `main`** — the job deploys with `--branch main`,
and Cloudflare only treats a deployment as production when that matches, so a
mismatch silently publishes every release to a preview URL instead. Two secrets:
`CLOUDFLARE_API_TOKEN` (scoped to "Cloudflare Pages: Edit", nothing more) and
`CLOUDFLARE_ACCOUNT_ID`. `wrangler` is pinned, like every other tool the repo
installs.
...
- `assetPrefix` is already `"./"`, so a subdomain root needs no rebuild config;
  `PUBLIC_BASE_PATH` exists only for subpath hosting. The bundle is genuinely
  path-agnostic — the same files work at a domain root, under a subpath, or over
