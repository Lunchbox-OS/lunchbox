...
- A **VA-API driver** at runtime, for hardware video decoding. `mpv` does not
  ...
  the right one depends on the GPU, so `lunchbox-admin va-api install` picks it
  from the detected hardware (`va-api detect` shows what it found).
  `lunchbox deps install run` and `lunchbox-admin media-deps install` both
...
`LUNCHBOX_MEDIA_VIDEO_CACHE_MAX_BYTES` (a byte count) still overrides it, as a
...
starting point — copy it to `~/.config/lunchbox/movies.toml` and update the
...
`$XDG_CACHE_HOME/lunchbox/media/playlists/<list-id>.json` for 6 hours, so
...
[#2](https://github.com/Lunchbox-OS/lunchbox/issues/2) and
[#10](https://github.com/Lunchbox-OS/lunchbox/issues/10).
...
lunchboxd-side support for that is tracked separately; for now `lunchbox-media`
...
`lunchbox-admin config validate` instead of on the child's screen.
...
Set **`LUNCHBOX_MPV_HWDEC`** to take a different path: `auto-safe` for zero-copy
...
`$XDG_STATE_HOME/lunchbox/media/resume/<library_id>.toml` (falling back to
...
Buckets are cached under `$XDG_CACHE_HOME/lunchbox/media/sponsorblock/` for a
