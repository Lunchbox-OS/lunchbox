...
let mut server = IpcServer::new("/run/lunchboxd/lunchboxd.sock");
server.start().await?;
...
The peer's uid decides nothing, because every activity `lunchboxd` launches runs
as `lunchboxd`'s own uid. The identity that does separate them is the peer's
...
| Peer | Verdict |
|------|---------|
| In `lunchboxd`'s own cgroup | accepted as `Admin` |
...
sudo systemd-run --uid=1000 --gid=1000 --scope --slice=user-1000.slice \
  ...
  ./target/debug/lunchboxd -c ./config.example.toml -s /tmp/t/s.sock -d /tmp/t/data
```
...
and the one-shots connect (they share lunchboxd's cgroup, as on a device), a
...
| Attempt | Result |
|---------|--------|
| Socket in a root-owned directory | `lunchboxd` cannot bind there at all — `EACCES` |
...
passes the fd — which `lunchboxd` cannot use while sway `exec`s it.
...
`(st_dev, st_ino)` against what it bound, and `lunchboxd` polls it once a minute
...
let mut client = IpcClient::connect("/run/lunchboxd/lunchboxd.sock").await?;
```

### Sending Commands

```rust
use lunchbox_api::{Command, Response};
...
- `lunchbox-api` - Message types
- `lunchbox-util` - Client IDs
