...
//!
//! Speaks the JSON-RPC wire format defined in `lunchbox_api::Request`
...
/// IPC Client for connecting to lunchboxd
...
impl IpcClient {
    /// Connect to lunchboxd, refusing anything that is not this session's own
    /// daemon (issue #144).
    ///
    ...
    /// path. No file mode prevents that — see [`lunchbox_ipc::ServerCheck`] —
        ...
        match crate::classify_server(stream.as_fd()) {
            ...
            ServerCheck::Foreign { server, ours } => {
                warn!(
                    ...
                    our_cgroup_id = ours,
                    "Something other than this session's lunchboxd answered on the management \
    ...
    pub async fn list_windows(&mut self) -> IpcResult<Vec<lunchbox_api::WindowInfo>> {
        self.call::<Vec<lunchbox_api::WindowInfo>>("list_windows", Value::Null)
            .await
    }
    ...
    pub async fn act_on_window(
        &mut self,
        id: u64,
        action: lunchbox_api::WindowAction,
    ) -> IpcResult<()> {
        self.call::<Value>(
            "act_on_window",
    ...
    pub async fn list_desktop_apps(&mut self) -> IpcResult<Vec<lunchbox_api::DesktopApp>> {
        self.call::<Vec<lunchbox_api::DesktopApp>>("list_desktop_apps", Value::Null)
            .await
    }
    ...
    pub async fn get_hud_orientation(&mut self) -> IpcResult<lunchbox_api::HudOrientation> {
...
/// `lunchbox_management::LaunchOutcome` serializes. It was previously
...
/// Stream of events from lunchboxd
...
mod tests {
    ...
    // tests under `crates/lunchbox-e2e/`.
    //
    ...
    /// The exact bytes lunchboxd puts on the wire, captured from a headless
