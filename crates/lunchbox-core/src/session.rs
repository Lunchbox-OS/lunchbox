...
use lunchbox_api::{SessionEndReason, SessionState, WarningThreshold};
use lunchbox_host_api::HostSessionHandle;
use lunchbox_util::{EntryId, MonotonicInstant, SessionId};
...
pub struct SessionPlan {
    ...
    /// [`lunchbox_api::EntryKind::supports_reset`]. Derived from the entry's
    /// kind at launch so the HUD knows whether to offer the button.
    pub can_reset: bool,
    /// Whether the HUD should offer page-turn buttons — see
    /// [`lunchbox_api::EntryKind::supports_page_turn`]. Derived the same way,
    ...
    pub hud_orientation: Option<lunchbox_api::HudOrientation>,
}
...
impl ActiveSession {
    ...
    pub fn to_session_info(&self, now_mono: MonotonicInstant) -> lunchbox_api::SessionInfo {
        lunchbox_api::SessionInfo {
...
mod tests {
    use super::*;
    use lunchbox_api::WarningSeverity;
