...
use lunchbox_api::{ReasonCode, ServiceStateSnapshot};
use lunchbox_ipc::{IpcClient, LaunchOutcome};
...
impl ServiceClient {
    ...
    async fn connect_and_run(&mut self) -> Result<()> {
        ...
        info!(path = %self.socket_path.display(), "Connecting to lunchboxd");

        let mut client = IpcClient::connect(&self.socket_path)
            .await
            .context("Failed to connect to lunchboxd")?;

        info!("Connected to lunchboxd");
        ...
        loop {
            tokio::select! {
                ...
                // Handle events from lunchboxd
                event_result = events.next() => {
                    match event_result {
                        Ok(event) => {
                            info!(event = ?event, "Received event from lunchboxd (client.rs)");
...
impl CommandClient {
    ...
    pub async fn stop_current(&self) -> Result<()> {
        let mut client = IpcClient::connect(&self.socket_path).await?;
        client
            .stop_current(lunchbox_api::StopMode::Graceful)
            .await
            .map_err(Into::into)
    }

    /// The applications administrator mode's picker offers (issue #154).
    pub async fn list_desktop_apps(&self) -> Result<Vec<lunchbox_api::DesktopApp>> {
        let mut client = IpcClient::connect(&self.socket_path).await?;
    ...
    /// Tell lunchboxd the seat is idle; it leaves administrator mode only if
    ...
    #[allow(dead_code)]
    pub async fn list_entries(&self) -> Result<Vec<lunchbox_api::EntryView>> {
        let mut client = IpcClient::connect(&self.socket_path).await?;
