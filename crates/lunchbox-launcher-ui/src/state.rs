...
pub enum LauncherState {
    ...
    /// that lunchboxd broadcasts on resume replaces it (issue #73).
    Suspending,
}
...
impl SharedState {
    ...
    /// Update state from lunchboxd event
    pub fn handle_event(&self, event: Event) {
        tracing::info!(event = ?event.payload, "Received event from lunchboxd");
        match event.payload {
            ...
            EventPayload::SystemResumed => {
                // Keep the cover up; the StateChanged lunchboxd broadcasts on
            ...
            EventPayload::DisplayModeChanged { .. } => {
                // External-display arrangement is handled by lunchboxd and the
