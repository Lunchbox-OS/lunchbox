...
//! the value into the JSON Schema, which `lunchbox-wire-codegen` renders into
...
//! `lunchbox-wire-codegen` fails until someone does run it.
//!
...
/// [`WarningThreshold`](lunchbox_api::WarningThreshold), because the editor
...
pub struct DefaultWarning {
    ...
    /// [`WarningSeverity`](lunchbox_api::WarningSeverity).
...
/// string the editor's own types reject.
fn severity_wire_name(severity: lunchbox_api::WarningSeverity) -> &'static str {
    use lunchbox_api::WarningSeverity as S;
    match severity {
        S::Info => "info",
        S::Warn => "warn",
        S::Critical => "critical",
