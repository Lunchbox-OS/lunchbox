...
//! (`shepherd_util::ProtectedFile`). A file manager cannot take that advice —
...
//! kiosk user: every activity runs at shepherdd's own uid and can read and
...
mod tests {
    ...
    fn denied_subtrees_are_refused_however_they_are_reached() {
        let (_d, root) = fixture();
        let denied = vec![root.join(".local/share/shepherdd")];
        fs::create_dir_all(root.join(".local/share/shepherdd")).unwrap();
        fs::write(root.join(".local/share/shepherdd/shepherd.db"), b"db").unwrap();
        assert!(resolve(&root, ".local/share/shepherdd/shepherd.db", &denied).is_err());
        ...
        symlink(root.join(".local/share/shepherdd"), root.join("state")).unwrap();
