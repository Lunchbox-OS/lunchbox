//! shepherd-tablet-bridge: translate absolute pointer / tablet input into
//! synthetic touch events.
//!
//! The inverse of shepherd-touch-bridge. Run it alongside an activity that
//! only handles touch (and ignores mouse/pen input); the bridge grabs every
//! absolute pointing device — a graphics tablet or digitizer, or the absolute
//! "tablet" a VM exposes (QEMU/SPICE present the guest pointer this way) — and
//! re-emits its motion as `wl_touch` contacts through a `/dev/uinput` virtual
//! touchscreen. A contact is held while a button (`BTN_TOUCH`, or a mouse
//! `BTN_LEFT`) is down, so click-drag becomes a touch stroke and plain motion
//! (a hovering pen, or a moved-but-unclicked cursor) emits nothing — touch has
//! no hover. The uinput backend works on any Wayland compositor (and X11),
//! like the other sidecars (issue #58).
//!
//! Devices reporting `INPUT_PROP_DIRECT` — real touchscreens and our own
//! synthetic touchscreen — are skipped: they already deliver touch, and
//! grabbing our own output would feed back on itself. Releasing the grabs is
//! handled by the kernel when the file descriptors close at process exit.
...
/// Single-finger model: every synthesized contact uses slot 0. The source
/// devices we target (a VM tablet or a pen) report one point at a time.
const SLOT: u32 = 0;

#[derive(Parser, Debug)]
#[command(
    name = "shepherd-tablet-bridge",
    about = "Grab absolute pointers/tablets and emit synthetic touch events"
)]
struct Args {
    /// Tablet / absolute-pointer device path. Repeat for multiple devices. If
    /// omitted, every non-touchscreen absolute pointer under /dev/input is
    /// auto-detected.
...
impl DeviceRange {
    ...
    /// for the touch events. Both fall back to safe defaults if the axis
...
/// True if the device is an absolute pointer or tablet we should re-map to
/// touch: it has absolute X/Y axes and a tip/click button, but is **not**
/// already a direct touch device (`INPUT_PROP_DIRECT`). The property check
/// excludes real touchscreens and our own synthetic touchscreen, so the
/// bridge can't grab its own output.
fn looks_like_tablet(dev: &Device) -> bool {
    if dev.properties().contains(PropType::DIRECT) {
        return false;
    }
    let has_abs = dev
    ...
    let has_contact_button = dev
        ...
        .unwrap_or(false);
    has_abs && has_contact_button
}

fn discover_tablets() -> Vec<PathBuf> {
    let mut paths = Vec::new();
    for (path, dev) in evdev::enumerate() {
        if looks_like_tablet(&dev) {
            debug!(path = %path.display(), name = %dev.name().unwrap_or(""), "Found tablet");
            paths.push(path);
        }
    }
    paths
...
/// Open a tablet, grab it, and start a reader thread. The thread sends
...
fn device_loop(mut dev: Device, path: PathBuf, tx: Sender<TouchUpdate>, shutdown: Arc<AtomicBool>) {
    ...
    let mut last_x = 0;
    let mut last_y = 0;
    // A contact is held whenever the tip or a mouse button is down. We track
    // both source buttons because real tablets report `BTN_TOUCH` for the tip
    // while VM absolute pointers (QEMU/SPICE) report `BTN_LEFT` for a click.
        ...
        for ev in events {
            match ev.destructure() {
                ...
                EventSummary::Synchronization(_, SynchronizationCode::SYN_REPORT, _) => {
                    ...
                    let target_contact = btn_touch || btn_left;
...
fn install_signal_handlers(shutdown: Arc<AtomicBool>) -> Result<()> {
    ...
    thread::Builder::new()
        .name("tablet-bridge-signals".into())
        .spawn(move || {
            loop {
...
fn main() -> Result<()> {
    ...
    if device_paths.is_empty() {
        return Err(anyhow!(
            "no tablet / absolute-pointer devices found; the user running this \
             bridge must be in the 'input' group, or pass --device explicitly"
        ));
    }
...
fn emit_update(sink: &mut UinputSink, range: DeviceRange, update: TouchUpdate) {
    ...
    match update {
        ...
        TouchUpdate::Up => {
            sink.dispatch(OutputEvent::TouchUp { slot: SLOT }, 0);
            sink.frame();
        }
    }
}
