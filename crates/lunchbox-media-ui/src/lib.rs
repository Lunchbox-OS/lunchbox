//! Shared egui UI for browsing a lunchbox-media library.
//!
//! Both the Linux binary (`lunchbox-media`) and the Android app
//! (`lunchbox-media-android`) render the same poster grid for viewing a
...
//! depends on `egui` and `glow` (and `lunchbox-media-core`) but not `eframe`.
//!
