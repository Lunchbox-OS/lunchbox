//! [`RemoteStore`] — the `Store` shepherdd uses when its state lives behind the
...
//! does, so nothing in `shepherd-core`, `shepherd-management` or
//! `shepherd-http` changes: they already hold an `Arc<dyn Store>`.
//!
...
use shepherd_store::{AuditEvent, StateSnapshot, Store, StoreError, StoreResult, TokenState};
    ...
    pub fn connect_at(socket: PathBuf, expect_uid: u32) -> std::io::Result<Self> {
        ...
        if reply.proto != PROTO_VERSION {
            return Err(std::io::Error::other(format!(
                "shepherd-stated speaks protocol {} and this build speaks {PROTO_VERSION}; \
...
/// name the way it can with shepherdd's own management socket
...
/// refusal rather than shepherdd trusting whatever answered.
///
/// Deliberately a *uid* check and not a cgroup one, unlike
/// `shepherd_ipc::classify_server`: the custodian is outside shepherdd's cgroup
...
/// `SO_PEERCRED`'s uid. Kept local rather than depending on `shepherd-ipc`,
...
/// A `Store` served by `shepherd-stated` over a Unix socket.
