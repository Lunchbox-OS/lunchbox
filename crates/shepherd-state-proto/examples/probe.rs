//! Hand-driven check that a real `shepherd-stated` answers a real `RemoteStore`.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let store = shepherd_state_proto::RemoteStore::connect(&user)?;
    println!("connected: {}", store.socket_path().display());
    println!("healthy: {}", store.is_healthy());
    let entry = shepherd_util::EntryId::new("probe-entry");
    let day = shepherd_util::now().date_naive();
