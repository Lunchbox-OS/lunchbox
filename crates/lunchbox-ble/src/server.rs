...
//! [`lunchbox_api::Event`] from [`ManagementService::subscribe_events`]
...
use lunchbox_management::ManagementService;
use std::borrow::Cow;
...
pub struct BleServerConfig {
    ...
    pub files: Arc<dyn lunchbox_util::ProtectedFiles>,
}
...
impl BleServer {
    ...
    /// `lunchbox-http`) can consult the admin record for the unified
...
mod tests {
    ...
    fn server_with(sink: Arc<RecordingSink>) -> BleServer {
        ...
        let config = BleServerConfig {
            ...
            adapter: None,
            files: Arc::new(lunchbox_util::LocalProtectedFiles::new(
    ...
    fn only_whole_state_snapshots_coalesce() {
        let snapshot = lunchbox_api::ServiceStateSnapshot {
            api_version: 1,
            policy_loaded: true,
    ...
    fn sentinel_reset_captures_bond_for_removal() {
        ...
        let admin_path = dir
            .path()
            .join(lunchbox_util::ProtectedFile::AdminRecord.file_name());
        let sentinel_path = dir
            .path()
            .join(lunchbox_util::ProtectedFile::ResetSentinel.file_name());
