...
impl AdminRecord {
    ...
    ///
    /// [`AdminSummary`] lives in `lunchbox-management` because both transports
...
"#,
    ...
    fn pending_unbond_creates_missing_parent() {
        let dir = TempDir::new().unwrap();
        let root = dir.path().join("nested/sub");
        let store = PendingUnbondStore::new(Arc::new(lunchbox_util::LocalProtectedFiles::new(
            root.clone(),
        )));
        store.add("AA:BB:CC:DD:EE:FF").unwrap();
