...
Bluetooth LE management transport for lunchboxd.

This crate exposes `BleServer`, the BLE counterpart to `lunchbox-http`'s
`HttpServer`. Both serve the same `lunchbox_management::ManagementService`
...
is the same shape `lunchbox-management`'s `webauth` uses to let a browser in
...
- **The roster is not BLE's.** Listing administrators, approving a waiting
  ...
  `lunchbox_management::AdminRoster`, which `ClaimMachine` implements. They are
...
the state custodian in `/var/lib/lunchboxd/admin/` — one directory for the
machine, shared by every kiosk user on it. The policy and the usage database are
per-user, under `/var/lib/lunchboxd/state/<user>/`, because those are facts
about a child.
...
`lunchboxd`s would then contend for one adapter's GATT registration.
