...
//! helper. `shepherd-firewall-helper stop-scope` existed for exactly this and
...
//!   * `/usr/libexec/shepherd-firewall-helper` installed
//!   * polkit granting `org.shepherd.firewall.apply-process` without a prompt
...
async fn stop_scope_tears_down_a_firewalled_activity() {
    ...
    let pid = match handle.payload() {
        shepherd_host_api::HostHandlePayload::Linux { pid, .. } => *pid,
