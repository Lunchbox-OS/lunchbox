...
//!    interface is associated with. `shepherdd` already depends on this bus
...
//! NetworkManager is non-fatal" bargain `shepherdd`'s system-event watcher
...
use shepherd_host_api::{NetworkInfoProvider, NetworkSnapshot};
...
mod tests {
    ...
    ///
    /// `cargo test -p shepherd-host-linux -- --ignored --nocapture reads_this_device`
    #[tokio::test]
