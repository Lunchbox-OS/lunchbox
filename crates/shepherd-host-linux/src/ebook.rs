...
//! [`EntryKind::wants_polite_close`](shepherd_api::EntryKind::wants_polite_close).
//!
...
//! 3. **A local XMLGUI `.rc`** hides the toolbar, which is not a config setting
//!    at all — `KToolBar` reads visibility from the `hidden` attribute of the
//!    GUI definition. See [`render_toolbar_rc`].
//!
...
use shepherd_api::{EbookLayout, EbookViewer, EntryKind};
use tracing::debug;
...
pub const EBOOK_ROOT_ENV: &str = "SHEPHERD_EBOOK_ROOT";
...
fn root_dir() -> PathBuf {
    ...
    let root = match crate::helpers::env_override(EBOOK_ROOT_ENV) {
        Some(root) => root,
        None => shepherd_util::default_data_dir().join("ebook"),
    };
...
pub fn render_kdeglobals(kiosk: bool) -> String {
    if !kiosk {
        return "# shepherd: kiosk = false, no restrictions applied\n".to_string();
    }

    let mut out = String::from(
        "# Generated by shepherd (issue #160). Re-rendered on every launch.\n\
...
/// Render an XMLGUI definition that hides a toolbar.
///
/// Toolbar visibility is not a config key: `KToolBar` reads it from the
/// `hidden` attribute of the GUI definition (`kxmlgui/src/ktoolbar.cpp`), and
/// the local override lives at `$XDG_DATA_HOME/kxmlgui5/<component>/<file>.rc`.
///
/// Rather than ship a copy of Okular's own (150-line, version-stamped) GUI
/// definition, which would go stale on every upgrade and take the menus with
/// it, this emits a *minimal* document containing nothing but the toolbar
/// declaration. XMLGUI merges it with the application's own definition, and a
/// merge that finds no actions to place still carries the attribute.
///
/// `version` is deliberately high so the local file wins the merge; KXMLGUI
/// compares versions and takes the newer.
fn render_toolbar_rc(component: &str, toolbar: &str) -> String {
    format!(
        "<?xml version=\"1.0\"?>\n\
         <!-- Generated by shepherd (issue #160): hides the toolbar, which is not a\n\
              config setting. Re-rendered on every launch. -->\n\
         <gui name=\"{component}\" version=\"1000\">\n\
         \x20<ToolBar name=\"{toolbar}\" hidden=\"true\">\n\
         \x20 <text>Main Toolbar</text>\n\
         \x20</ToolBar>\n\
         </gui>\n"
    )
}

/// The XMLGUI files shepherd writes for Okular: the shell's toolbar and the
/// part's, which are separate GUI definitions merged into one window.
const OKULAR_TOOLBAR_RCS: &[(&str, &str, &str)] = &[
    ("okular", "shell.rc", "mainToolBar"),
    ("okular_part", "part.rc", "mainToolBar"),
];
...
fn materialize(paths: &Paths, spec: &Spec<'_>) -> io::Result<()> {
    ...
    for (component, file, toolbar) in OKULAR_TOOLBAR_RCS {
        let dir = paths.data.join("kxmlgui5").join(component);
        std::fs::create_dir_all(&dir)?;
        if spec.kiosk {
            std::fs::write(dir.join(file), render_toolbar_rc(component, toolbar))?;
        } else {
            // An unrestricted run gets the reader's own GUI back, rather than
            // whatever a previous kiosk launch left behind.
...
mod tests {
    ...
    #[test]
    fn toolbar_rc_declares_the_toolbar_hidden() {
        let rendered = render_toolbar_rc("okular_part", "mainToolBar");
        assert!(rendered.contains("<gui name=\"okular_part\""));
        assert!(rendered.contains("<ToolBar name=\"mainToolBar\" hidden=\"true\">"));
    }
    ...
    fn prepare_writes_config_and_builds_argv() {
        ...
        assert!(
            launch
                .paths
                .data
                .join("kxmlgui5/okular_part/part.rc")
                .exists()
        );
    }
    ...
    #[test]
    fn turning_kiosk_off_removes_the_toolbar_override() {
        let scratch = tempfile::tempdir().unwrap();
        let book = scratch.path().join("hobbit.epub");
        std::fs::write(&book, b"x").unwrap();
        let root = scratch.path().join("state");

        let locked = spec_for(&book, &[]);
        let launch = prepare_in(&root, &locked, Some("book"), |s| s.to_string()).unwrap();
        let part_rc = launch.paths.data.join("kxmlgui5/okular_part/part.rc");
        assert!(part_rc.exists());

        let mut open = locked;
        open.kiosk = false;
        prepare_in(&root, &open, Some("book"), |s| s.to_string()).unwrap();
        assert!(!part_rc.exists());
    }
