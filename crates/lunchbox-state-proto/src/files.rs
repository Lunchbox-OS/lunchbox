...
//!
//! It implements [`ProtectedFiles`], which is defined in `lunchbox-util` rather
//! than here on purpose — `lunchbox-ble` keeps the admin record and must not
...
///
/// Replaces the `notify` watcher lunchboxd used to run on its own config
...
/// better answer than this function does — lunchboxd re-reads its policy on the
...
impl ConfigWatch {
    ...
    /// signal rather than work — lunchboxd sends on a channel.
    pub fn start(
        user: &str,
