...
- Boot: lunchboxd comes up, `POST /api/v1/rpc { "method": "health" }` reports ready, the IPC socket
...
`--test-threads=1` so the per-test sway/lunchboxd processes don't fight
over PIDs and ports).

## In CI

The `e2e` job in `.github/workflows/ci.yml` installs the test deps, builds
the binaries, and runs `cargo test -p lunchbox-e2e -- --include-ignored
