...
fn workspace_binary(name: &str) -> PathBuf {
    // CARGO_MANIFEST_DIR is `<workspace>/crates/lunchbox-e2e`.
...
impl HarnessBuilder {
    ...
    /// Add an environment variable to lunchboxd's process. Applied after the
    ...
    /// lunchboxd as a child, so the two share a cgroup and every client is
...
impl TestHarness {
    ...
    async fn start(builder: HarnessBuilder) -> Result<Self> {
        ...
        fs::write(&config_path, &rendered_config).context("write lunchboxd config")?;
        ...
        lunchboxd_cmd
            ...
            .arg("-d")
            .arg(&data_dir)
            // lunchboxd unlinks sway's IPC socket by default (issue #144). This
            ...
            // lunchboxd otherwise accepts a client on its own socket only from
            // its own cgroup (issue #144). This harness spawns lunchboxd as a
            ...
            // exercised by the unit tests in `lunchbox-ipc`, which can put a
            ...
            // lunchboxd with an explicit `-d <temp dir>`, so it keeps the
        ...
        wait_for("lunchboxd rpc health", Duration::from_secs(15), || {
        ...
        wait_for("lunchboxd ipc socket", Duration::from_secs(5), || {
    ...
    pub async fn connect_ipc(&self) -> Result<lunchbox_ipc::IpcClient> {
        Ok(lunchbox_ipc::IpcClient::connect(&self.socket_path).await?)
    }
    ...
    pub async fn shutdown(mut self) -> Result<()> {
        self.shut_down = true;
        // Reverse order: UIs, then lunchboxd, then sway.
        for slot in [
            &mut self.hud,
            &mut self.launcher,
            &mut self.lunchboxd,
