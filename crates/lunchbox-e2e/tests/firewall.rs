...
//! these tests target the *wiring* that `lunchboxd` does on top:
//!
...
async fn firewall_supported_path_invokes_helper_with_expected_argv() -> Result<()> {
    ...
    // pkcheck → always grant. lunchboxd runs this once during process::init().
    ...
    // lunchbox-firewall-helper → record argv (one element per line, with
    ...
    write_executable(
        &stubs_path.join("lunchbox-firewall-helper"),
        &format!(
            "#!/bin/sh\n\
             {{\n\
                 printf '%s\\n' 'ARGV_BEGIN'\n\
    ...
    let helper_path = stubs_path.join("lunchbox-firewall-helper");
