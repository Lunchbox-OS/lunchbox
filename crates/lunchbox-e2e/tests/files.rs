...
//!
//! `crates/lunchbox-http/tests/` covers the routes in process, which is where
//! the protocol belongs. Three things only a real `lunchboxd` can show:
//!
...
use lunchbox_e2e::{HttpClient, TestHarness, json_body};
...
fn fake_home() -> Result<tempfile::TempDir> {
    ...
    // layout on a device. (The harness starts lunchboxd with an explicit
    ...
    // `crates/lunchbox-http/tests/files.rs`.)
    std::fs::create_dir_all(home.path().join(".local/share/lunchboxd"))?;
    std::fs::write(
        home.path().join(".local/share/lunchboxd/lunchboxd.db"),
        b"sqlite",
    )?;
...
async fn the_daemon_will_not_hand_over_its_own_state() -> Result<()> {
    ...
    let listed = http
        .get("/api/v1/files/list?root=home&path=.local%2Fshare%2Flunchboxd")
        .await?;
    ...
    // And the log is deliberately *not* refused: pulling lunchboxd.log off a
