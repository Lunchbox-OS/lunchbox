...
```rust
use lunchbox_util::{EntryId, SessionId, ClientId};
...
```rust
use lunchbox_util::MonotonicInstant;
...
```rust
use lunchbox_util::{default_socket_path, default_data_dir, default_log_dir};

// Get XDG-compliant paths (no root required)
let socket = default_socket_path();  // $XDG_RUNTIME_DIR/lunchboxd/lunchboxd.sock
let data = default_data_dir();       // $XDG_DATA_HOME/lunchboxd or ~/.local/share/lunchboxd
