...
//!
//! shepherdd used to watch its own config directory with inotify and reload on
...
//!
//! Watching the **directory** rather than the file, exactly as shepherdd did:
...
/// boot. shepherdd will say something far more useful about a missing config
...
pub fn warn_if_no_policy(dir: &Path) {
    ...
    if !policy.exists() {
        warn!(
            path = %policy.display(),
            "No policy file in the protected directory; shepherdd will read its own until \
