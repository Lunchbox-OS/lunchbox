...
//! sent back to shepherdd — which is alive at that moment, and has a
...
impl Authority {
    ...
    /// What to tell shepherdd, when there is something to tell it.
        ...
        match self {
            ...
            Self::Denied => Some(format!(
                "polkit refuses {ACTION} to this daemon's uid, so the session watchdog cannot \
                 end the session when shepherdd stops supervising it. Install \
