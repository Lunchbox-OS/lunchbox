...
Every other shepherd surface is GTK4 (`shepherd-hud`, `shepherd-launcher-ui`,
`shepherd-pairing-display`). This one is a raw `wayland-client` +
...
shepherdd spawns this process (`HostAdapter::set_locked(true)`) and unlocks it by
...
the session restarts. That is the safe direction, and it is why shepherdd always
asks politely and waits for the exit rather than killing on a timeout.

A crashed lock **is** recoverable without a reboot: a fresh `shepherd-lock` can
take the lock again and then release it, which is what pressing "unlock" does
after a crash. Verified end to end.

## How shepherdd finds it, and why that has to be got right
...
it falls back to the bare name, resolved through `shepherd_host_linux::helpers`
...
an activity could satisfy by writing its own `shepherd-lock` onto a
...
the sibling probe looks in `/usr/bin`, where `shepherdd` lives. So this crate
...
```
failed to start the screen lock at /usr/bin/shepherd-lock: No such file or
directory (os error 2)
```

while every development session locked perfectly, because `target/debug` had it
sitting next to `shepherdd` the whole time. Adding it to that list is also what
...
./target/debug/shepherd-lock &        # covers the screen
./scripts/shepherd dev shot lock.png  # see it
pkill -TERM shepherd-lock             # releases it
```
