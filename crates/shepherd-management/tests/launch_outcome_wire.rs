...
//! tagged (`{"Approved": {…}}`) while `shepherd_ipc::LaunchOutcome` was
...
use shepherd_management::LaunchOutcome as ServerOutcome;

/// Serialize exactly as shepherdd does when answering `launch`.
...
fn client_decodes_an_approval_from_the_server() {
    ...
    match serde_json::from_str::<shepherd_ipc::LaunchOutcome>(&wire) {
        Ok(shepherd_ipc::LaunchOutcome::Approved { session_id, .. }) => {
...
fn client_decodes_a_denial_from_the_server() {
    ...
    match serde_json::from_str::<shepherd_ipc::LaunchOutcome>(&wire) {
        Ok(shepherd_ipc::LaunchOutcome::Denied { reasons }) => {
            assert_eq!(
                reasons.len(),
                2,
