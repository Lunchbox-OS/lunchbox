...
//! to lunchboxd over a socket that admits exactly one cgroup: the kiosk's logind
...
//! may launch something — that stays in `lunchbox-core`, inside lunchboxd. This
...
//! lunchboxd's own socket because it lives at the child's uid, and concluded
//! that preventing it "requires the socket to be created by something other than
//! the daemon, i.e. systemd socket activation … which `lunchboxd` cannot use
...
use clap::Parser;
use lunchbox_ipc::{PeerPolicy, kernel_supports_peer_cgroup};
use lunchbox_state_proto::StateRequest;
use lunchbox_store::{SqliteStore, Store};
use lunchbox_util::{LocalProtectedFiles, ProtectedFiles};
use std::sync::Arc;
...
#[derive(Parser, Debug)]
#[command(name = "lunchbox-stated", about, version)]
struct Args {
...
async fn main() -> Result<()> {
    ...
    info!(user = %args.user, uid, "lunchbox-stated starting");

    // Refuse before binding rather than after. A kernel that cannot report a
    // peer's cgroup cannot tell lunchboxd from an activity, and a broker that
    // cannot tell them apart has nothing to offer: serving anyway would hand
    // over exactly what this exists to protect. lunchboxd's own check makes the
    ...
    // The session watchdog (issue #172). Armed by lunchboxd opening a
    ...
    // supervision connection: lunchboxd is the only process that can report
            ...
            (
                ...
                Some(format!(
                    "this daemon could not reach logind to end a session ({e}), so nothing \
                     will end it if lunchboxd stops supervising it"
                )),
            )
        }
    };
    ...
    let db_path = state_dir.join("lunchboxd.db");
    ...
    // The same `LocalProtectedFiles` lunchboxd uses for its data directory when
    ...
    // by this uid at 0700.
    let admin_dir = lunchbox_state_proto::admin_dir();
...
/// Where the systemd unit's `StateDirectory=lunchboxd/state/%i` puts things.
///
...
fn default_state_dir(user: &str) -> std::path::PathBuf {
    // The same constant lunchboxd stats to tell "the custodian is broken" apart
    // from "this device never had one", so the two cannot drift into disagreeing
    // about where the state lives.
    lunchbox_state_proto::state_dir(user)
}
...
fn listener(args: &Args) -> Result<UnixListener> {
    match inherited_listener()? {
        ...
        None => {
            ...
            // inode, so the mode has to be permissive for lunchboxd — at a
            ...
            // lunchboxd out) or group-shared with the kiosk uid, which every
...
async fn serve(
    ...
    loop {
        ...
        // and the same reason as lunchboxd's own socket.
        let uid = lunchbox_ipc::peer_uid(stream.as_fd());
        ...
        if matches!(
            ...
            loop {
                match changes.recv().await {
                    Ok(()) => {
                        let line = serde_json::to_string(&lunchbox_state_proto::ConfigChanged {})?;
        ...
        let reply = match serde_json::from_str::<StateRequest>(&line) {
            Ok(request) => {
                ...
                tokio::task::spawn_blocking(move || {
                    lunchbox_state_proto::server::handle(store.as_ref(), files.as_ref(), request)
                })
                .await
            ...
            Err(e) => {
                ...
                // a bug in lunchboxd, not an attack — answer with an error
                // rather than dropping the connection, so it says so.
                warn!(error = %e, "Could not decode a request");
                serde_json::to_string(&lunchbox_state_proto::WireResult::<()>::Err {
                    kind: lunchbox_state_proto::WireErrorKind::Serialization,
