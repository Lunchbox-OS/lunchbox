...
//!
//! `lunchboxd` runs as the kiosk uid, and so does every activity. Signal
...
//! lunchboxd into `loginctl terminate-session`, runs at that uid too. Kill the
...
///
/// lunchboxd beats every five seconds — a quarter of this, capped — so twelve
...
/// a whole daemon startup in front of it: lunchboxd opens this channel while it
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Reason {
    /// Nothing holds a supervision connection any more: lunchboxd exited or
    /// was killed.
    Gone,
...
impl std::fmt::Display for Reason {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Gone => write!(f, "lunchboxd is gone"),
            Self::Silent => write!(f, "lunchboxd stopped sending heartbeats"),
        }
    }
}
...
impl Handle {
    ...
    pub fn reply(&self) -> lunchbox_state_proto::SuperviseReply {
        lunchbox_state_proto::SuperviseReply {
            armed: self.armed,
...
pub trait Terminator: Send + Sync + 'static {
    ...
    /// `lunchbox-firewall-helper::lifetime_args`).
...
mod tests {
    ...
    fn nothing_fires_before_anything_supervises() {
        // The custodian is reachable by root as well as by lunchboxd, so it can
    ...
    fn a_second_connection_does_not_disarm_on_the_first_close() {
        // Two supervision connections is not an arrangement lunchboxd uses, but
    ...
    fn a_slow_startup_is_not_a_dead_daemon() {
        // lunchboxd opens this channel while it is still starting: a database
