...
`lunchboxd` runs inside the kiosk session, as the same uid as every activity it
...
`/var/lib/lunchboxd/admin/`, while the policy and the database live under
...
File permissions cannot separate them, because `lunchboxd` runs at the uid it
...
`lunchboxd` over a socket that admits exactly one cgroup.
...
│ sway → lunchboxd → the one connection above              │
└──────────────────────────────────────────────────────────┘
...
comparison `lunchbox-ipc`'s `PeerPolicy` makes for `lunchboxd`'s own socket, via
...
[`2026-08-29 004`]'s finding 2 showed an activity can `unlink()` `lunchboxd`'s
...
passing the fd, which `lunchboxd` cannot use while sway `exec`s it".

This daemon can, so it does. `/run/lunchboxd/state/` is root-owned; the kiosk
...
socket needs *write* permission on the inode, and `lunchboxd` is at a different
uid. **The peer check is the gate; the file mode is not.**

## It also ends the session when nothing is supervising it (issue [#172])

Every activity runs as the kiosk uid, and so does `lunchboxd`. Signal permission
is a uid comparison, so an activity can `kill` its own supervisor — and the
`sh -c` wrapper that turns a dead `lunchboxd` into `loginctl terminate-session`
runs at that uid too, so killing it first defeats it. `kill -STOP lunchboxd`
...
```
lunchboxd ──Supervise──▶ custodian     "will anything happen if I die?"
...
only that *a thread* is alive. A `lunchboxd` whose engine has stopped and whose
...
message**: every orderly shutdown also ends with `lunchboxd` gone and the
...
| | |
| --- | --- |
| wrappers killed first, then `lunchboxd` | session gone in **5.10 s**; sway, the activity and its scope with it |
| `kill -STOP lunchboxd` | session gone in **56.9 s**, with the `\|\|` fallback's own processes alive the whole time |
...
| **a slow boot** | the first beat gets two minutes rather than the deadline's one: this channel opens while `lunchboxd` is still starting, and its engine tick runs after that. Nothing is unguarded meanwhile — an activity cannot launch before the engine that would launch it |
...
everything else still works; `lunchboxd` raises the `Critical` diagnostic
...
systemctl status lunchbox-stated@kiosk.service  # runs once lunchboxd connects
journalctl -u lunchbox-stated@kiosk.service     # what it trusted, and what it refused
```
...
and `lunchboxd` ran on an unprotected local store. Its fallback is a
...
verified on an installed kiosk after a clean boot: `lunchboxd` connects to the
...
```
lunchboxd:      State served by the custodian; it is not reachable by activities
activity:       ls /var/lib/lunchboxd/state/kiosk/   -> Permission denied
...
and the fallback `lunchboxd` reads when the custodian is unreachable — with a
...
daemon serves and lunchboxd cannot parse is fatal at *startup* (tolerated only
