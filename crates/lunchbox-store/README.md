...
let store = SqliteStore::open("/var/lib/lunchboxd/lunchboxd.db")?;
```
...
```rust
use lunchbox_store::{AuditEvent, AuditEventType};
...
-- Rows are created by discovery: lunchboxd records each output it observes so
