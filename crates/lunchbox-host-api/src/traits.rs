...
pub struct SpawnOptions {
    ...
    /// lunchboxd already knows it, and the activity must not have to be told
    /// twice. A media activity and lunchboxd's prefetcher write to one cache
    ...
    /// `media_watched_grace_days`: the activity trims the cache lunchboxd
...
/// Mirrors `lunchbox-config`'s validated `BrowserPolicy`, kept separate so the
...
pub trait HostAdapter: Send + Sync {
    ...
    /// connection lunchboxd already holds.
    ...
    ///
    /// On Linux this runs `lunchbox-lock`, an `ext-session-lock-v1` client, and
...
/// - `XwaylandHidpi` in `lunchboxd` — the production sway-backed
...
///
/// Two implementations ship in the workspace:
/// - `HudLayout` in `lunchboxd` — the production implementation, which
...
pub trait HudLayoutController: Send + Sync {
    ...
    /// stands.
    async fn apply(&self, orientation: Option<lunchbox_api::HudOrientation>);
    ...
    async fn orientation(&self) -> lunchbox_api::HudOrientation;
}
...
impl HudLayoutController for NoOpHudLayoutController {
    async fn apply(&self, _orientation: Option<lunchbox_api::HudOrientation>) {}
    async fn restore(&self) {}
    async fn orientation(&self) -> lunchbox_api::HudOrientation {
...
///
/// Two implementations ship in the workspace:
/// - `DisplayManager` in `lunchboxd` — the production sway-backed implementation
