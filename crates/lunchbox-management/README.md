...
Transport-agnostic management service for lunchboxd.

This crate defines `ManagementService`, the trait that captures every
operation an administrator can perform on a running lunchboxd: list
...
It exists so the HTTP transport (`lunchbox-http`) and the planned BLE
transport (`lunchbox-ble`, see
...
- `#[management_rpc]` builds a `dispatch_json` arm for each `async` method and
  ...
  surface would be a method that exists and cannot work. `lunchbox-http`
...
policy and `config_path` otherwise — the same choice `lunchboxd` makes at boot,
...
It lives in this crate rather than in `lunchbox-http` because `lunchbox-http`
...
`lunchbox-http` because the companion reaches two of those three over BLE —
...
(`POST /api/v1/auth/login` and friends, in `lunchbox-http`); a transport that
