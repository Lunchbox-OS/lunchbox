...
use lunchbox_api::{EntryKind, Event, EventPayload, StopMode};
use lunchbox_config::{
...
struct Harness {
    ...
    /// Whether lunchboxd has been asked to end the desktop session. Held here
...
/// (`lunchbox-launcher-ui/src/app.rs`), so those presses never go through the
...
async fn the_session_reports_itself_as_stopping_during_teardown() {
    ...
    assert_eq!(
        ...
        Some(lunchbox_api::SessionState::Running)
    );
    ...
    assert_eq!(
        h.svc.current_session().await.map(|s| s.state),
        Some(lunchbox_api::SessionState::Stopping),
...
async fn a_launch_that_never_started_is_not_charged() {
    ...
    let ended = {
        ...
        eng.notify_launch_failed(
            Some(&handle),
            "never started".into(),
            lunchbox_util::MonotonicInstant::now(),
            lunchbox_util::now(),
        )
    };
...
async fn time_before_the_window_appears_is_not_charged() {
    ...
    {
        let mut eng = h.svc.engine.lock().await;
        eng.notify_window_ready(&handle, lunchbox_util::MonotonicInstant::now());
    }
...
async fn stale_exit_from_previous_activity_does_not_end_the_next_session() {
    ...
    // Now tetris's exit finally surfaces. This is the call `lunchboxd`'s
...
async fn locking_on_the_idle_timeout_does_not_log_out() {
    let h = harness();
    h.svc.enter_admin_mode().await.unwrap();
    h.host.set_windows(vec![lunchbox_api::WindowInfo {
        id: 1,
        ...
        visible: true,
        focused: true,
        owner: lunchbox_api::WindowOwner::Unowned,
    }]);
