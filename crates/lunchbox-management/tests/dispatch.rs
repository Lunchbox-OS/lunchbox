...
struct MockVolume {
    ...
    output: std::sync::Mutex<Option<lunchbox_api::AudioOutput>>,
    ...
    present: std::sync::Mutex<Vec<lunchbox_api::AudioOutput>>,
...
impl MockVolume {
    ...
    fn plug_in(&self, o: lunchbox_api::AudioOutput) {
...
impl VolumeController for MockVolume {
    ...
    /// a single `pw-dump`.
    async fn observe(&self) -> VolumeResult<lunchbox_host_api::AudioSnapshot> {
        if !*self.readable.lock().unwrap() {
            return Err(lunchbox_host_api::VolumeError::Backend(
                "could not read the audio topology".into(),
            ));
        }
        Ok(lunchbox_host_api::AudioSnapshot {
    ...
    async fn select_output(&self, output_key: &str) -> VolumeResult<()> {
        ...
        match found {
            ...
            None => Err(lunchbox_host_api::VolumeError::NotAvailable(format!(
...
fn output(key: &str, description: &str) -> lunchbox_api::AudioOutput {
    lunchbox_api::AudioOutput {
        key: key.into(),
        description: description.into(),
        kind: lunchbox_api::AudioOutputKind::Unknown,
    }
}
...
/// for `/var/lib/lunchboxd/state/<user>/`.
///
...
async fn write_policy_is_audited() {
    ...
    assert!(
        audits.iter().any(|a| matches!(
            a.event,
            lunchbox_store::AuditEventType::PolicyWritten { entry_count: 1 }
        )),
        "{audits:?}"
    );
}
...
fn next_volume_event(rx: &mut broadcast::Receiver<Event>) -> Option<(u8, bool, Option<String>)> {
    while let Ok(ev) = rx.try_recv() {
        if let lunchbox_api::EventPayload::VolumeChanged {
            percent,
            muted,
            output,
            ..
...
async fn audio_watch_reports_a_volume_change_made_behind_our_back() {
    ...
    // Something outside lunchboxd moved the volume — a bare `wpctl` call, or a
...
struct RecordingSink {
    raised: std::sync::Mutex<Vec<lunchbox_api::DiagnosticCode>>,
    cleared: std::sync::Mutex<Vec<lunchbox_api::DiagnosticCode>>,
}

impl lunchbox_api::DiagnosticSink for RecordingSink {
    fn raise(&self, diagnostic: lunchbox_api::Diagnostic) {
        self.raised.lock().unwrap().push(diagnostic.code);
    }
    fn clear(&self, code: lunchbox_api::DiagnosticCode, _s: &lunchbox_api::DiagnosticSubject) {
...
async fn a_failed_topology_read_is_reported_to_the_parent_and_clears_itself() {
    ...
    svc.diagnostics = Some(sink.clone() as Arc<dyn lunchbox_api::DiagnosticSink>);
    ...
    assert!(
        sink.raised
            .lock()
            .unwrap()
            .contains(&lunchbox_api::DiagnosticCode::AudioTopologyUnreadable),
...
async fn volume_event_carries_the_restrictions_in_force() {
    ...
    let lunchbox_api::EventPayload::VolumeChanged { restrictions, .. } = ev.payload else {
