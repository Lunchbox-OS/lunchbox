...
//! revoking an old one. Both live here rather than in `lunchbox-ble` so the
...
///
/// Implemented by `lunchbox-ble`'s `ClaimMachine`, which owns the records, the
...
/// [`crate::ManagementService`], and this crate sits below `lunchbox-ble` and
/// cannot name it.
///
