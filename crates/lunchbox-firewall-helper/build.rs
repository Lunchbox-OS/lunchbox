...
fn main() {
    ...
    let status = Command::new("rustup")
        ...
        .env_remove("RUSTDOC")
        .status()
        .expect("spawn `rustup run nightly cargo` for lunchbox-firewall-bpf");
    assert!(status.success(), "lunchbox-firewall-bpf build failed");

    let bpf_obj = bpf_dir.join("target/bpfel-unknown-none/release/lunchbox-firewall-bpf");
    assert!(
        bpf_obj.exists(),
