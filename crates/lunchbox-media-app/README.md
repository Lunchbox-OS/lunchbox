...
On Linux, `lunchbox-media` is (almost) stateless: `lunchboxd` spawns it per
...
no `lunchboxd` and only one install per device, so it must keep its own state:
...
- **Caching policy** (`quality.rs`) — [`CacheMode`] (mirrors the two
  `VideoCache` strategies in `lunchbox-media`, plus `off`), [`PosterPolicy`],
...
Turning a [`LibraryEntry`] into a resolved `lunchbox_media_core::Library` needs
