...
use lunchbox_state_proto::{RemoteFiles, RemoteStore, Supervision};
use lunchbox_store::{AuditEvent, AuditEventType, SqliteStore, Store};
use lunchbox_util::{
...
struct BleManagement {
    ...
    authority: Option<Arc<dyn lunchbox_management::AdminAuthority>>,
    roster: Option<Arc<dyn lunchbox_management::AdminRoster>>,
}
...
/// lunchboxd - Policy enforcement service for child-focused computing
#[derive(Parser, Debug)]
...
struct Args {
    ...
    /// hardening leaves nothing able to reach the compositor except lunchboxd
    ...
    /// sway session lunchboxd happens to be inside: run by hand in a
    ...
    /// Accept a client on lunchboxd's own management socket from any process
    /// at this uid, instead of only from the session lunchboxd is part of.
    ///
    ...
    /// Accepted peers are those in lunchboxd's own cgroup — the launcher, the
    ...
    /// lunchboxd's cgroup is one an activity cannot join, which is true of a
...
struct Service {
    ...
    /// both how it is watched (lunchboxd cannot inotify a directory it cannot
...
enum IpcPeerHardening {
    ...
    OptedOut,
    /// Armed, but lunchboxd is somewhere the check cannot hold, or could not
...
/// a dropped [`lunchbox_state_proto::ConfigWatch`] closes its connection.
enum PolicyWatch {
    ...
    #[allow(dead_code)]
    Custodian(lunchbox_state_proto::ConfigWatch),
    None,
}
...
impl Service {
    ...
    fn custodian_holds_state_for(user: &str) -> bool {
        lunchbox_state_proto::state_dir(user).is_dir()
    }
        ...
        if Self::custodian_holds_state_for(user) {
            anyhow::bail!(
                "the state custodian is installed for {user} and did not answer ({reason}); \
                 ...
                 record are in /var/lib/lunchboxd, not where an unprotected run would look. \
                 Check `systemctl status lunchbox-stated@{user}.service` and \
                 `journalctl -u lunchbox-stated@{user}.service`"
            );
        }
        warn!(
            error = %reason,
    ...
    fn connect_with_retries(user: &str) -> Result<RemoteStore, lunchbox_store::StoreError> {
    ...
    fn warn_about_a_superseded_local_store(data_dir: &Path) {
        let stale = data_dir.join("lunchboxd.db");
        if stale.exists() {
            warn!(
                path = %stale.display(),
    ...
    fn watch_policy(
        ...
        if custodial {
            ...
            return match lunchbox_state_proto::ConfigWatch::start(
                &user,
                move || {
    ...
    fn open_local_store(data_dir: &Path) -> Result<Arc<dyn Store>> {
        let db_path = data_dir.join("lunchboxd.db");
    ...
    async fn new(args: &Args) -> Result<Self> {
        ...
        lunchbox_host_linux::helpers::set_trust_environment(args.trust_environment);
        ...
        lunchbox_media_cache::set_scope_prefix_fn(lunchbox_host_linux::helper_scope_argv_prefix);
        ...
        lunchbox_media_cache::set_program_resolver_fn(lunchbox_host_linux::resolve_helper_arg);
    ...
    async fn harden_compositor_socket(
        ...
        if let Some(alias) = alias {
            match lunchbox_host_linux::sway_ipc::alias_socket(alias) {
        ...
        match lunchbox_host_linux::sway_ipc::unlink_socket() {
            Ok(()) => info!(
    ...
    /// Arm the peer allow-list on lunchboxd's own management socket (#144).
    ///
    /// Every activity runs as lunchboxd's uid, so the socket's mode separates
    ...
    fn arm_ipc_peer_policy(ipc: &mut IpcServer, restrict: bool) -> IpcPeerHardening {
        ...
        let policy = match lunchbox_ipc::PeerPolicy::restricted() {
            Ok(policy) => policy,
            Err(e) => {
        ...
        match lunchbox_ipc::own_cgroup_path() {
            ...
            Ok(path) => {
                ...
                match lunchbox_host_linux::activity_isolation_status() {
                    lunchbox_host_linux::ActivityIsolationStatus::Supported => {
    ...
    fn ipc_peer_rejected_diagnostic(rejection: &lunchbox_ipc::Rejection) -> Diagnostic {
    ...
    async fn run(mut self) -> Result<()> {
        ...
        // ends the session — which is what an exiting lunchboxd wants anyway.
        ...
        // Shutdown signal: any path that should bring down lunchboxd flips this
        ...
        // same lunchbox_management::ManagementService, so the service is
        ...
        // authentication.
        let web_auth: Option<Arc<lunchbox_management::WebAuth>> = match &management_api_config {
            Some(cfg) => {
                let policy = lunchbox_management::WebAuthPolicy {
                ...
                match lunchbox_management::WebAuth::load(Arc::clone(&protected_files), policy) {
        ...
        let BleManagement {
            ...
            Some(ble_cfg) => {
                ...
                // `lunchbox-pairing-display` is spawned per pairing
                    ...
                    Ok(server) => {
                        let server = server
                            .with_diagnostics(Arc::new(diagnostic_publisher.clone())
                                as Arc<dyn lunchbox_api::DiagnosticSink>);
                        ...
                        let authority =
                            claim.clone() as Arc<dyn lunchbox_management::AdminAuthority>;
                        let roster = claim as Arc<dyn lunchbox_management::AdminRoster>;
            ...
            Some((api_cfg, web)) => {
                ...
                let file_manager = file_manager_config
                    ...
                    .map(|home| {
                        ...
                        Arc::new(
                            lunchbox_http::FileService::new(home, file_manager_rx.clone())
        ...
        //
        // When the custodian holds it, lunchboxd cannot use inotify: it cannot
    ...
    async fn handle_config_reload(
        ...
        let loaded = match policy_files {
            Some(files) => files
                .read(ProtectedFile::Config)
                .map_err(lunchbox_config::ConfigError::ReadError)
                .and_then(|text| match text {
                    Some(text) => lunchbox_config::parse_config(&text),
                    None => Err(lunchbox_config::ConfigError::ReadError(
                        std::io::Error::new(
    ...
    async fn sweep_diagnostics(
        ...
        let fresh = diagnostics::evaluate(&facts, lunchbox_util::now());
    ...
    ///
    /// Requests are dispatched through `lunchbox_management::dispatch_json`
    ...
    async fn handle_ipc_message(
        ...
        match msg {
            ServerMessage::Request { client_id, request } => {
                ...
                if request.api_version != lunchbox_api::API_VERSION {
                    let resp = Response::error(
                        request.request_id,
                        ErrorInfo::new(
...
async fn dispatch_ipc(
    ...
    match lunchbox_management::dispatch_json(svc, method, params).await {
        Ok(value) => Response::success(request_id, value),
        Err(lunchbox_management::RpcDispatchError::MethodNotFound(m)) => Response::error(
            request_id,
            ErrorInfo::new(ErrorCode::MethodNotFound, format!("unknown method '{m}'")),
        ),
        Err(lunchbox_management::RpcDispatchError::InvalidParams(msg)) => {
            Response::error(request_id, ErrorInfo::new(ErrorCode::InvalidParams, msg))
        }
        Err(lunchbox_management::RpcDispatchError::Serialization(msg)) => {
            Response::error(request_id, ErrorInfo::new(ErrorCode::Internal, msg))
        }
        Err(lunchbox_management::RpcDispatchError::Management(e)) => {
            let (code, msg) = match e {
                lunchbox_management::ManagementError::NotFound(m) => (ErrorCode::NotFound, m),
                lunchbox_management::ManagementError::BadRequest(m) => (ErrorCode::BadRequest, m),
                lunchbox_management::ManagementError::Forbidden(m) => (ErrorCode::Forbidden, m),
                lunchbox_management::ManagementError::Conflict(m) => (ErrorCode::Conflict, m),
                lunchbox_management::ManagementError::Unprocessable(m) => {
                    (ErrorCode::Unprocessable, m)
                }
                lunchbox_management::ManagementError::Internal(m) => (ErrorCode::Internal, m),
            };
...
fn announce_web_auth_state(
    web: &lunchbox_management::WebAuth,
    cfg: &lunchbox_config::ManagementApiConfig,
) {
...
async fn main() -> Result<()> {
    ...
    info!(version = env!("CARGO_PKG_VERSION"), "lunchboxd starting");
...
mod policy_watch_tests {
    ...
    /// anyone else running lunchboxd with a relative `-c`.
    #[test]
...
mod harden_diagnostic_tests {
    ...
    fn an_unprotected_state_store_is_a_critical_service_condition() {
        let d = Service::state_not_protected_diagnostic(
            "connecting to the state custodian at /run/lunchboxd/state/kiosk.sock: \
    ...
    fn a_custodian_state_directory_is_what_says_protection_was_expected() {
        ...
        assert_eq!(
            lunchbox_state_proto::state_dir("kiosk"),
            std::path::Path::new("/var/lib/lunchboxd/state/kiosk")
        );
    }
}
