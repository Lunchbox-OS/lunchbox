//! [`PairingDisplay`] backed by the `lunchbox-pairing-display`
//! subprocess.
//!
//! The BLE pairing agent (in lunchbox-ble) is `Send + Sync` and calls
//! [`show`](lunchbox_ble::PairingDisplay::show) /
//! [`hide`](lunchbox_ble::PairingDisplay::hide) synchronously. We
...
use lunchbox_ble::{PairingDisplay, PairingMethod};
...
const BINARY: &str = "lunchbox-pairing-display";

/// `PairingDisplay` implementation that fans out to the
/// `lunchbox-pairing-display` subprocess.
...
impl PairingDisplay for SwayPairingDisplay {
    fn show(&self, device_address: &str, passkey: u32, method: PairingMethod) {
        ...
        let mut cmd = Command::new(lunchbox_host_linux::resolve_daemon_sibling(BINARY));
        cmd.args([
            "--passkey",
            &passkey.to_string(),
            "--device",
        ...
        // any GTK / Wayland errors) lands in lunchboxd's journal.
