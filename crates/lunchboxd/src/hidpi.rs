...
//! with `xwayland_native_resolution = true` is running, lunchboxd drops
...
//! lunchboxd broadcasts the captured pre-launch scale as a `HudScaleChanged`
//! event so the HUD can apply a counter-scale and stay readable.

use crate::display::DisplayManager;
use async_trait::async_trait;
use lunchbox_api::{Event, EventPayload};
use lunchbox_host_api::HidpiController;
use lunchbox_host_linux::{OutputScale, get_outputs, set_output_scale};
...
/// into lunchbox-http's `AppState` (where the IPC server and broadcast
/// channel aren't directly visible) and the IPC handlers in lunchboxd.
...
impl HidpiController for XwaylandHidpi {
    ...
    async fn apply(&self) {
        ...
        if (factor - 1.0).abs() < f64::EPSILON && !outputs.is_empty() {
            ...
            // never ran (lunchboxd restarted mid-session, or `set_output_scale`
