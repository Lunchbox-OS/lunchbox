...
//!
//! Run as `cargo run -p lunchbox-wire-codegen --bin rpc-codegen`
...
fn main() -> anyhow::Result<()> {
    let schema: Schema = serde_json::from_str(lunchbox_management::RPC_SCHEMA_JSON)?;
    let pretty = serde_json::to_string_pretty(&serde_json::from_str::<Value>(
        lunchbox_management::RPC_SCHEMA_JSON,
    )?)?;
...
fn render_wire_types_ts() -> String {
    let schema = lunchbox_wire_codegen::wire_schema::wire_schema();
    lunchbox_wire_codegen::ts_types::render(&schema, WIRE_TS_PREAMBLE)
}
...
fn render_config_types() -> String {
    let schema = lunchbox_wire_codegen::config_schema::config_schema();
    ...
    lunchbox_wire_codegen::ts_types::render(&defs, CONFIG_TS_PREAMBLE)
}
...
fn render_editor_types() -> String {
    let schema = lunchbox_wire_codegen::editor_schema::editor_schema();
    lunchbox_wire_codegen::ts_types::render(&schema, EDITOR_TS_PREAMBLE)
}

/// Kotlin mirrors of the payload types, rendered from the wire JSON Schema.
fn render_wire_types() -> String {
    lunchbox_wire_codegen::kotlin_types::render(&lunchbox_wire_codegen::wire_schema::wire_schema())
}
...
fn render_kotlin_params(schema: &Schema) -> String {
    ...
    for (i, m) in schema.methods.iter().enumerate() {
        ...
        let fn_name = lunchbox_wire_codegen::kotlin_types::camel(&m.name);
        ...
        let args: Vec<String> = m
            ...
            .iter()
            .map(|p| {
                let name = lunchbox_wire_codegen::kotlin_types::camel(&p.name);
