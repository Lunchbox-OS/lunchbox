...
/// Build the `lunchbox-media` command line for an [`EntryKind::Media`] entry
/// (issue #127).
///
/// Every flag is passed explicitly rather than relying on `lunchbox-media`'s
...
/// activity writes to the same cache directory lunchboxd prefetches into, and
...
fn media_argv(
    ...
    let EntryKind::Media {
        ...
        reverse,
        resume,
        // Prefetch is lunchboxd's business, not the player's: it never reaches
        // the command line.
        prefetch: _,
    ...
    if let Some(item) = item {
        ...
        // is set can't produce an invocation `lunchbox-media` would refuse.
...
pub struct LinuxHost {
    ...
    /// The running `lunchbox-lock`, when the screen is locked (issue #154).
    ///
    ...
    pub fn preload_steam(&self) {
        ...
        let scope = match activity_isolation_status() {
            ...
            ActivityIsolationStatus::Unsupported { reason } => {
                ...
                // leave Steam entries gated forever. lunchboxd already raises
        ...
        Ok(tokio::spawn(async move {
            loop {
                let (_, body) = match subscription.next_event().await {
                    Ok(event) => event,
                    Err(e) => {
                        // lunchboxd is exec'd by sway and dies with it, so a
                ...
                if let Some(sink) = sink {
                    sink.raise(Diagnostic {
                        ...
                        message: "shepherd cannot see the compositor, so it cannot tell what is \
                            ...
                            .to_string(),
                        remedy: Some(
                            "Check that lunchboxd is running inside the sway session and that \
...
impl HostAdapter for LinuxHost {
    ...
    async fn spawn(
        ...
        // or it inherits lunchboxd's cgroup and becomes indistinguishable from
        ...
        // Put the activity in a cgroup that is not lunchboxd's, so the peer
    ...
    async fn set_locked(&self, locked: bool) -> HostResult<()> {
        ...
        match (locked, slot.as_mut()) {
            ...
            (true, None) => {
                ...
                // own `lunchbox-lock` on a PATH the kiosk user chooses — and
                // this particular binary is the thing standing between a child
                // and a locked screen.
                let exe = std::env::current_exe()
                    .ok()
                    .and_then(|p| p.parent().map(|d| d.join("lunchbox-lock")))
                    .filter(|p| p.exists())
                    .map(|p| p.to_string_lossy().into_owned())
                    .unwrap_or_else(|| "lunchbox-lock".into());
                // The resolved path, not the name, because the failure worth
                // naming is "it is not installed" and the two look identical
                // otherwise: a device that never shipped `lunchbox-lock` and one
            ...
            (false, Some(child)) => {
                // SIGTERM, never SIGKILL: `lunchbox-lock` handles it by
    ...
    async fn launch_unsupervised(&self, argv: &[String]) -> HostResult<()> {
        ...
        // (issue #144). The management socket trusts lunchboxd's cgroup, and a
        ...
        // program shares lunchboxd's process group and dies with it, which
...
mod tests {
    use super::*;
    use lunchbox_api::{MediaMode, MediaQuality, MediaSortBy};
    ...
    fn media_argv_forwards_the_watched_grace() {
        // The activity writes to the same video cache lunchboxd prefetches
    ...
    fn media_argv_forwards_the_cache_cap() {
        // Same reason as the grace: the activity trims the cache lunchboxd
