...
//!   afternoon. Lunchbox does **not** relocate it: it lands wherever the user's
...
use lunchbox_api::{EntryKind, RetroarchSaveState};
...
/// is used.
pub const RETROARCH_ROOT_ENV: &str = "LUNCHBOX_RETROARCH_ROOT";
...
///
/// `LUNCHBOX_LIBRETRO_DIR` and `LUNCHBOX_RETROARCH_CONFIG_DIR` both document a
...
///
/// `LUNCHBOX_RETROARCH_ROOT` used to be in that set. It is not any more: its
...
fn root_dir() -> PathBuf {
    ...
    let root = match crate::helpers::env_override(RETROARCH_ROOT_ENV) {
        Some(root) => root,
        None => lunchbox_util::default_data_dir().join("retroarch"),
    };
...
/// passing one rather than exporting `LUNCHBOX_RETROARCH_ROOT`. That variable
...
pub const LIBRETRO_DIR_ENV: &str = "LUNCHBOX_LIBRETRO_DIR";
...
pub fn render_append_config(paths: &Paths, save_state: RetroarchSaveState, kiosk: bool) -> String {
    ...
    out.push_str(
        "# Generated by lunchbox-launcher. Rewritten on every launch, so edits\n\
...
/// so an override that names any of these silently wins over lunchbox. Two are
...
pub const RETROARCH_CONFIG_DIR_ENV: &str = "LUNCHBOX_RETROARCH_CONFIG_DIR";
