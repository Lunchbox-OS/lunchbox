...
The bridge is launched by `lunchbox-host-linux` when an activity has a
...
1. Uses [gilrs](https://docs.rs/gilrs) for gamepad enumeration and event
   reading — the same library `lunchbox-launcher-ui` uses for the
...
```
lunchbox-gamepad-bridge --preset (productivity|gpd) [options]
```
