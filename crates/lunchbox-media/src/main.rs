//! Linux entry point for the `lunchbox-media` library launcher.
...
use clap::Parser;
use lunchbox_media_app::{ResumeStore, ResumeTracker};
...
use lunchbox_media_cache::{SponsorBlockCache, VideoCache};
use lunchbox_media_core::sponsorblock::Category;

/// Exit codes per the spec; keep in sync with `docs/lunchbox-media.md`.
const EXIT_OK: u8 = 0;
...
fn main() -> ExitCode {
    ...
    let watched_grace = lunchbox_media_cache::grace_from_days(cli.watched_grace_days);
...
fn load_library_from_source(
    ...
    let mut library = if is_youtube_playlist_url(source) {
        let info =
            lunchbox_media_cache::fetch_playlist(source).map_err(|e| (EXIT_VALIDATION, e))?;
...
fn build_session(
    ...
    let player: Box<dyn lunchbox_media_core::PlayerHandle> = match cache {
