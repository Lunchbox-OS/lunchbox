...
//! [`RemotePosterCache`] from `lunchbox-media-app`, used identically by the
//! Android app. Only the HTTP fetch and the XDG cache-dir choice are here.

use std::collections::HashMap;
use std::time::Duration;

use lunchbox_media_app::poster_cache::{DEFAULT_TTL, RemotePosterCache, Resolution};
use lunchbox_media_core::{Library, PosterRef};
...
///
/// Networking happens here, in the binary, so that `lunchbox-media-core`
...
pub fn prefetch(library: &Library) -> PosterCache {
    ...
    let disk = lunchbox_media_cache::media_cache_dir("posters")
