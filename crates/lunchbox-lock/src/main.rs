//! `lunchbox-lock` — the screen lock for administrator mode (issue #154).
//!
...
//! # Unlocking
//!
//! lunchboxd spawns this process and unlocks by sending **SIGTERM**, which is
...
//! the safe direction, and it is why lunchboxd must always ask politely.
...
impl SessionLockHandler for App {
    ...
    fn finished(&mut self, _conn: &Connection, _qh: &QueueHandle<Self>, _lock: SessionLock) {
        ...
        // client already holds one. Exiting non-zero tells lunchboxd the screen
