...
| `GET`  | `/api/v1/events` | Server-Sent Events stream of every `lunchbox_api::Event`. |
...
regenerated by `cargo run -p lunchbox-management --bin rpc-codegen`, and every
...
check. A policy lunchboxd cannot parse is survivable on *reload* and fatal at
*startup*, which on a device is a session that ends.

**The write does not reload.** It lands through a rename, which the state
custodian's watch — or lunchboxd's own, on a device without one — turns into a
...
book, a ROM or a video has to go into — while `lunchboxd` already runs *as*
that user.
...
- **Serve Lunchbox's own state, or an SSH key.** `~/.local/share/lunchboxd`
  ...
  `~/.local/state/lunchboxd` is *not* refused: pulling `lunchboxd.log` off a
...
claimed. `lunchboxd` now always builds a `WebAuth` store alongside the API, and
...
`GET /api/v1/events` streams the full `lunchbox_api::Event` firehose:
...
The HTTP server is started by lunchboxd when `[service.management_api].enabled
= true`. Config knobs:
...
2. **Over SSH** — delete the credential store and restart the daemon. A fresh
   ...
   ```sh
   sudo rm /var/lib/lunchboxd/admin/web-auth.toml
   sudo systemctl restart lunchboxd    # or restart the dev stack
   ```
...
`lunchbox-management/src/webauth.rs`. The e2e crate
(`crates/lunchbox-e2e`) drives the same endpoint end-to-end against a real
lunchboxd process.
...
`crates/lunchbox-e2e/tests/files.rs` is the last layer: a real `lunchboxd`,
