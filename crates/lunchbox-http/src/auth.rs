...
//! lunchboxd runs the API, and an unconfigured store answers the setup
...
use lunchbox_management::{AdminAuthority, WebAuth};
...
impl AuthSources {
    ...
    /// authenticates it. The pre-#156 shape, kept for the `lunchbox-http`
    /// tests and for an embedding that wants the management trait over HTTP
    /// without the login machinery.
    ///
    /// Never correct on a device. `lunchboxd` cannot reach it:
