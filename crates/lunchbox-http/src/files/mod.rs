...
//! child's home. lunchboxd already runs *as* that user, so it is the one
...
pub struct FileService {
    /// lunchboxd's own home directory, canonicalised once.
    home: PathBuf,
...
impl FileService {
    ...
    /// which is a guess: `lunchboxd -d /srv/library` puts it somewhere else
...
///
/// `~/.local/state/lunchboxd` is deliberately *not* here. It is the log
/// directory, and pulling `lunchboxd.log` off a device that has no shell is
