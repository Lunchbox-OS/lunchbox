...
/// Build the list. `home` is lunchboxd's own home directory.
pub fn enumerate(home: &Path, settings: &lunchbox_config::FileManagerConfig) -> Vec<RootInfo> {
...
/// Effective write access for the uid lunchboxd runs as.
///
...
/// and lunchboxd is not root. Deliberately not shared with
/// `lunchboxd::media::free_space`, which walks up to an existing ancestor
...
/// `crates/lunchbox-e2e/tests/removable_real.rs`.
...
";
    ...
    fn the_home_root_is_described() {
        ...
        let settings = lunchbox_config::FileManagerConfig::default();
    ...
    fn a_configured_root_that_is_not_there_is_not_listed() {
        let dir = tempfile::tempdir().unwrap();
        let settings = lunchbox_config::FileManagerConfig {
            external_media: false,
            extra_roots: vec![
                lunchbox_config::FileManagerRoot {
                    label: "Gone".into(),
