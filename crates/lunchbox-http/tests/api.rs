//! Integration tests for the lunchbox-http management API.
//!
//! Every operation goes through the single `POST /api/v1/rpc` endpoint
//! (see `handlers/rpc.rs`) into `lunchbox_management::dispatch_json`.
//! The *behavior* of each method — business logic, param parsing,
//! result wrapping — is tested once, transport-free, in
//! `lunchbox-management/tests/dispatch.rs`. What's left here is only
...
use lunchbox_http::{AppState, handlers};
...
fn make_app_with_admin(
    auth_token: Option<&str>,
    config_path: PathBuf,
    admin: Option<Arc<dyn lunchbox_management::AdminAuthority>>,
) -> axum::Router {
...
fn make_app_full(
    ...
    config_path: PathBuf,
    admin: Option<Arc<dyn lunchbox_management::AdminAuthority>>,
    web: Option<Arc<lunchbox_management::WebAuth>>,
    secure_cookies: bool,
) -> axum::Router {
    ...
    // fail-open state -- and `lunchboxd` cannot build it at all.
    let sources = match web {
        Some(web) => lunchbox_http::AuthSources::new(web),
        None => lunchbox_http::AuthSources::without_credential_store(),
    };
    handlers::router(
        state,
        sources
...
fn make_state_full(
    ...
    let svc = Arc::new(DefaultManagementService {
        ...
        shutdown_tx,
        hidpi: Arc::new(lunchbox_host_api::NoOpHidpiController),
        hud_layout: Arc::new(lunchbox_host_api::NoOpHudLayoutController),
        display: Arc::new(lunchbox_host_api::NoOpDisplayController),
    ...
    AppState {
        svc: svc as Arc<dyn lunchbox_management::ManagementService>,
...
async fn auth_admin_authority_token_passes() {
    use lunchbox_management::AdminAuthority;
    use std::sync::Mutex;
...
// in `lunchbox-management/src/webauth.rs`. What is left here is what only the
...
fn web_store(dir: &tempfile::TempDir) -> Arc<lunchbox_management::WebAuth> {
    Arc::new(
        lunchbox_management::WebAuth::load(
            Arc::new(lunchbox_util::LocalProtectedFiles::new(
                dir.path().to_path_buf(),
            )),
            lunchbox_management::WebAuthPolicy::default(),
        )
        .expect("store loads"),
    )
}
...
fn make_web_app(
    web: Arc<lunchbox_management::WebAuth>,
    config_path: PathBuf,
    auth_token: Option<&str>,
) -> axum::Router {
    make_app_full(
...
async fn enrol(app: &axum::Router, web: &lunchbox_management::WebAuth) -> String {
...
/// nothing checked it: `lunchboxd` built a store next to the server and the
...
async fn a_server_with_no_credential_store_refuses_to_serve() {
    ...
    let api_cfg = lunchbox_config::ManagementApiConfig {
        ...
        tls: lunchbox_config::TlsMode::Off,
        auth: lunchbox_config::WebAuthLimits::default(),
    };
    ...
    let outcome = tokio::time::timeout(
        ...
        lunchbox_http::HttpServer::new(state, api_cfg).run(rx),
    )
    .await
...
async fn lockout_answers_429_with_retry_after() {
    ...
    let web = Arc::new(
        lunchbox_management::WebAuth::load(
            ...
            lunchbox_management::WebAuthPolicy {
                lockout_after: 2,
