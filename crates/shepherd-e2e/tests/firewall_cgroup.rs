//! Does `shepherd-firewall-helper apply-cgroup` actually filter packets?
//!
...
//!
//! Run as root: `sudo -E cargo test -p shepherd-e2e --test firewall_cgroup --
...
fn repo_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .expect("crates/shepherd-e2e has a repo root")
        .to_path_buf()
}
...
fn helper_path() -> Option<PathBuf> {
    ...
    let root = repo_root();
    [
        root.join("target/debug/shepherd-firewall-helper"),
        root.join("target/release/shepherd-firewall-helper"),
        PathBuf::from("/usr/libexec/shepherd-firewall-helper"),
    ]
    .into_iter()
    .find(|p| p.exists())
}
...
fn cgroup_firewall_filters_real_packets() -> Result<()> {
    ...
    let Some(helper) = helper_path() else {
        return skip("shepherd-firewall-helper not built or installed");
    };
