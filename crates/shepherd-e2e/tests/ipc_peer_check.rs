//! #144's acceptance test: a peer shepherdd did not start cannot call a
//! mutating method over the management socket, and the refusal is audited.
//!
//! The unit tests in `shepherd-ipc` cover `classify` against a socketpair; this
//! covers the daemon — a real shepherdd, its real socket, and a peer in a
...
/// The same probe, left in the harness's cgroup — shepherdd's own.
...
async fn a_peer_outside_shepherds_cgroup_cannot_call_a_mutating_method() -> Result<()> {
    ...
    let store = SqliteStore::open(h.data_dir().join("shepherdd.db"))?;
