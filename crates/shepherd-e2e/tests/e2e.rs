...
/// Launch a session, extend it via `extend_current`, then stop it.
#[tokio::test(flavor = "multi_thread")]
#[ignore]
async fn extend_session_via_http() -> Result<()> {
    ...
    assert_eq!(resp.status, 200, "launch body: {}", resp.body);
    let body = json_body(&resp)?;
    let original_deadline = body["Approved"]["deadline"].as_str().map(str::to_owned);
    assert!(
    ...
    let resp = http
        .rpc("extend_current", json!({ "seconds": 600 }))
        .await?;
...
/// entry surfaces as unavailable in `get_entry`.
#[tokio::test(flavor = "multi_thread")]
#[ignore]
async fn daily_override_disables_entry() -> Result<()> {
    ...
    let resp = http.rpc("get_entry", json!({ "id": "always-on" })).await?;
    assert_eq!(resp.status, 200);
    let body = json_body(&resp)?;
    ...
    let resp = http
        .rpc(
            "upsert_override",
            json!({ "id": "always-on", "availability": false }),
        )
        .await?;
    ...
    let resp = http
        .rpc("delete_override", json!({ "id": "always-on" }))
        .await?;
    assert_eq!(resp.status, 200);
