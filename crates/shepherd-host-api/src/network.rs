...
//! `shepherd-management` must not link a D-Bus client, and a status page has to
//! be testable without a NetworkManager to talk to.

use async_trait::async_trait;
use shepherd_api::{Connectivity, NetworkInterfaceView, NetworkSource};
...
/// once in [`shepherd_api::NetworkStatusView::new`], so a second host
