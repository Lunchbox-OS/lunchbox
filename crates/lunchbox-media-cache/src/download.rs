...
use lunchbox_media_app::lru::{Score, ScoreWeights};
...
pub fn download_worker(
    ...
    for req in rx {
        ...
        // prefetching lunchboxd and a playing lunchbox-media share this
...
fn download_youtube(
    ...
    // whatever the remote host sends back, and a direct child of lunchboxd is
    ...
    let output = crate::subprocess::ytdlp_command("ytdlp-download")
        .args([
            ...
            "--extractor-args",
            lunchbox_media_core::YOUTUBE_EXTRACTOR_ARGS,
            "--format",
            ytdl_format,
            "--output",
        ])
