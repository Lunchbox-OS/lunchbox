...
//! playing one, and lunchboxd's prefetcher warms them alongside the video it is
...
//! cache is split. The disk policy is `lunchbox_media_app::BucketStore` and the
//! request is [`lunchbox_media_core::sponsorblock::bucket_url`], both shared
...
use lunchbox_media_core::sponsorblock::{Category, Segment, bucket_url, parse_bucket, plan_skips};
...
mod tests {
    ...
    /// hand (`cargo test -p lunchbox-media-cache -- --ignored live_fetch`) when
