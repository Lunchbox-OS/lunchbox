...
//! reads it at play time and queues after a video finishes; lunchboxd will
...
//! is judged to be worth (see `lunchbox_media_app::lru`):
//!
...
use lunchbox_media_app::lru::ScoreWeights;
use lunchbox_media_core::resolver::resolve_source;
use lunchbox_media_core::{ClassifiedUri, Library, Source};
...
// reach into `lunchbox-media-app` for it.
pub use lunchbox_media_app::lru::{DEFAULT_WATCHED_GRACE, Score, ScoreWeights as CacheWeights};
...
///
/// A local override for debugging and for a `lunchbox-media` run by hand — the
/// deployment mechanism is `service.media.cache_max_bytes`, which lunchboxd
...
impl VideoCache {
    ...
    /// defaulted here: this directory is shared with lunchboxd's prefetcher, and
    ...
    /// other's trims. lunchboxd resolves both from policy and hands them to
...
mod tests {
    use super::*;
    use lunchbox_media_core::{PlayerHint, Source};
