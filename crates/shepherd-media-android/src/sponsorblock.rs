...
//!
//! The Linux binary's half of this is `shepherd-media`'s `skipping.rs`, and
//! everything that decides anything is shared: the wire format, the filtering
//! and the skip state machine are `shepherd_media_core::sponsorblock`, the disk
//! cache is `shepherd_media_app::BucketStore`, and the request URL is built by
