...
//! [`Library`](shepherd_media_core::Library).
//!
//! This is the platform-specific half the `shepherd-media-app` crate
...
use shepherd_media_app::LibrarySource;
use shepherd_media_core::{Library, LibraryError};
use thiserror::Error;
...
fn parse_toml(content: &str, display_path: &str) -> Result<Library, ResolveError> {
    Ok(shepherd_media_core::library::parse_library(
        content,
...
fn parse_m3u(content: &str, display_path: &str) -> Result<Library, ResolveError> {
    Ok(shepherd_media_core::playlist::parse_playlist(
        content,
        Path::new(display_path),
    )?)
}
