...
object RpcParams {
    ...
    fun stopCurrent(mode: StopMode? = null): JsonObject = buildJsonObject {
        put("mode", mode?.let { LunchboxJson.encodeToJsonElement(StopMode.serializer(), it) } ?: JsonNull)
    }
    ...
    fun setDisplayMode(mode: DisplayMode): JsonObject = buildJsonObject {
        put("mode", LunchboxJson.encodeToJsonElement(DisplayMode.serializer(), mode))
    }
    ...
    fun actOnWindow(id: Long, action: WindowAction): JsonObject = buildJsonObject {
        put("id", JsonPrimitive(id))
        put("action", LunchboxJson.encodeToJsonElement(WindowAction.serializer(), action))
    }
}
