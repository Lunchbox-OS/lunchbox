...
import com.lunchbox_os.companion.ble.ErrorCode
import com.lunchbox_os.companion.ble.RpcResponse
import com.lunchbox_os.companion.ble.LunchboxJson
...
class WireTest {
    ...
    private inline fun <reified T> decode(json: String): T =
        LunchboxJson.decodeFromString(json)
    ...
    fun `device info`() {
        val info = decode<DeviceInfo>(
            """{"protocol_version":1,"firmware_version":"0.1.0","claim_state":"unclaimed","device_name":"lunchbox"}""",
        )
    ...
    fun `window list decodes both wayland and xwayland shapes`() {
        ...
        assertEquals(WindowOwner.LUNCHBOX, windows[1].owner)
    }
    ...
    fun `window action serialises to the wire spelling act_on_window expects`() {
        assertEquals("\"close\"", LunchboxJson.encodeToString(WindowAction.serializer(), WindowAction.CLOSE))
        assertEquals("\"hide\"", LunchboxJson.encodeToString(WindowAction.serializer(), WindowAction.HIDE))
        assertEquals("\"show\"", LunchboxJson.encodeToString(WindowAction.serializer(), WindowAction.SHOW))
        assertEquals("\"focus\"", LunchboxJson.encodeToString(WindowAction.serializer(), WindowAction.FOCUS))
    }
    ...
    fun `a known enum value still round-trips by its wire name`() {
        ...
        assertEquals(
            "state_not_protected",
            LunchboxJson.encodeToString(DiagnosticCode.serializer(), diagnostic.code)
                .trim('"'),
        )
    }
