...
/**
 * Devices are keyed by [DeviceRecord.androidIdentifier] — the peer's
...
class DeviceRepositoryTest {
    ...
    private fun storeWith(initial: List<DeviceRecord>): AdminRecordStore =
    ...
    fun `re-pairing the same device replaces its record even though the RPA changed`() = runTest {
        ...
        val repo = DeviceRepository(store)
        repo.load()
    ...
    fun `load collapses duplicates an older build persisted, keeping the newest`() = runTest {
        ...
        val saved = slot<List<DeviceRecord>>()
    ...
    fun `displayName falls back to the device name, which is the device's own`() {
        ...
        val r = record("AA:BB:CC:DD:EE:FF", "58:AE:E5:03:C5:48", deviceName = "lunchbox")
        assertEquals("lunchbox", r.displayName)
        assertEquals("Playroom", r.copy(nickname = "Playroom").displayName)
        assertEquals("lunchbox", r.copy(nickname = "  ").displayName)
    }
}
